...
exports.auth = function (request, reply) {
    ...
    var entry = function () {
        ...
        if (['twitter', 'facebook', 'yahoo'].indexOf(request.params.network) === -1) {
            return reply(Hapi.error.internal('Unknown third party network authentication', request.params.network));
        }
    ...
    var twitter = function () {
        ...
        if (!request.query.oauth_token) {
            return request.server.app.twitterClient.getOAuthRequestToken(function (err, token, secret, authorizeUri, params) {
                ...
                if (err) {
                    return reply(Hapi.error.internal('Failed to obtain a Twitter request token', err));
                }
        ...
        if (!request.query.oauth_verifier) {
            return reply(Hapi.error.internal('Missing verifier parameter in Twitter authorization response'));
        }

        var credentials = request.session.get('twitter', true);
        if (!credentials) {
            return reply(Hapi.error.internal('Missing Twitter request token cookie'));
        }

        if (request.query.oauth_token !== credentials.token) {
            return reply(Hapi.error.internal('Twitter authorized request token mismatch'));
        }
        ...
        request.server.app.twitterClient.getOAuthAccessToken(credentials.token, credentials.secret, request.query.oauth_verifier, function (err, token, secret, params) {
            ...
            if (err) {
                return reply(Hapi.error.internal('Failed to obtain a Twitter access token', err));
            }

            if (!params.user_id) {
                return reply(Hapi.error.internal('Invalid Twitter access token response', err));
            }
    ...
    var facebook = function () {
        ...
        if (!facebookSession ||
            ...
            return reply(Hapi.error.internal('Missing Facebook state cookie'));
        }

        if (facebookSession.state !== request.query.state) {
            return reply(Hapi.error.internal('Facebook incorrect state parameter'));
        }
        ...
        facebookRequest('POST', '/oauth/access_token', body, function (err, data) {
            ...
            facebookRequest('GET', '/me?' + QueryString.stringify({ oauth_token: data.access_token }), null, function (err, data) {
                ...
                if (!data ||
                    ...
                    return reply(Hapi.error.internal('Invalid Facebook profile response', err));
                }
    ...
    var yahoo = function () {
        ...
        if (!request.query.oauth_token) {
            request.server.app.yahooClient.getOAuthRequestToken(function (err, token, secret, authorizeUri, params) {
                ...
                if (err) {
                    return reply(Hapi.error.internal('Failed to obtain a Yahoo! request token', err));
                }
        ...
        if (!request.query.oauth_verifier) {
            return reply(Hapi.error.internal('Missing verifier parameter in Yahoo authorization response'));
        }

        credentials = request.session.get('yahoo', true);
        if (!credentials) {
            return reply(Hapi.error.internal('Missing Yahoo request token cookie'));
        }

        if (request.query.oauth_token !== credentials.token) {
            return reply(Hapi.error.internal('Yahoo authorized request token mismatch'));
        }
        ...
        request.server.app.yahooClient.getOAuthAccessToken(credentials.token, credentials.secret, request.query.oauth_verifier, function (err, token, secret, params) {
            ...
            if (err) {
                return reply(Hapi.error.internal('Failed to obtain a Yahoo access token', err));
            }

            if (!params ||
                !params.xoauth_yahoo_guid) {

                return reply(Hapi.error.internal('Invalid Yahoo access token response', params));
            }
    ...
    env.api.clientCall('POST', '/oz/login', payload, function (err, code, payload) {
        ...
        env.api.clientCall('POST', '/oz/rsvp', { rsvp: payload.rsvp }, function (err, code, ticket) {
            ...
            Session.set(request, ticket, function (isValid, restriction) {
                ...
                if (!isValid) {
                    return reply(Hapi.error.internal('Invalid response parameters from API server'));
                }
